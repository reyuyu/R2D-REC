...
- 缓解模型对训练目标（尤其推荐 Gold SID 的 one-hot 目标）的过度自信，提升泛化与生成稳定性；
- 采用"第一轮正常、第二轮平滑"的分阶段 schedule：让模型先充分拟合任务结构，再在第二轮用平滑目标微调校准置信度，避免全程平滑拖慢收敛；
- 属于 Alpha 系列"只改一个变量"原则下的 loss 目标侧实验，与 Alpha-CoT（权重侧）、REC-PU / Mini-TopK（损失函数替换）互为对照。
