...
try:
    ...
    from .official_finegrained_v6_adapter import (
        adapt_events as adapt_official_finegrained_v6_events,
        captured_payload as official_finegrained_v6_payload,
        is_official_finegrained_v6_manifest,
    )
    from .official_sample8_v3_adapter import (
        adapt_events as adapt_official_sample8_v3_events,
        captured_payload as official_sample8_v3_payload,
        is_official_sample8_v3_manifest,
    )
...
def monitor_advantage_formula(manifest: dict[str, Any]) -> str | None:
    ...
    if is_official_sample8_v3_manifest(manifest):
        return "official_sample8_v3"
    if is_official_finegrained_v6_manifest(manifest):
        return "official_finegrained_v6"
...
def create_app(
    ...
    *,
        ...
        for path in run_paths():
            ...
            runs.append({
                ...
                "display_name": manifest.get(
                    "display_name",
                    "GRPO-3 User GRPO" if is_grpo3_user_formal_manifest(manifest)
                    else "GRPO-2 推荐 Think-only" if manifest.get("schema") == GRPO2_CONTINUED_SCHEMA
    ...
    def official_source_rows(selected: Path) -> dict[str, dict[str, Any]]:
        """Read Official prompt/gold fields only after verifying its source dataset SHA."""
        try:
            manifest_data = json.loads((selected / "manifest.json").read_text(encoding="utf-8"))
            if not (
                is_official_anticopy_v5_manifest(manifest_data)
                or is_official_finegrained_v6_manifest(manifest_data)
                or is_official_sample8_v3_manifest(manifest_data)
            ):
                return {}
            dataset_value = manifest_data.get("source_dataset_path") or manifest_data.get("dataset_path")
            dataset = Path(dataset_value).expanduser().resolve()
            dataset.relative_to(Path("/data/GRPO/data").resolve())
            expected_sha = str(
                manifest_data.get("source_dataset_sha256")
                or manifest_data.get("dataset_guard", {}).get("source_sha256")
            )
            stat = dataset.stat()
        except (KeyError, OSError, TypeError, ValueError, json.JSONDecodeError):
            return {}
        if dataset.suffix != ".jsonl" or re.fullmatch(r"[0-9a-f]{64}", expected_sha) is None:
            return {}
        cache_key = (str(dataset), stat.st_mtime_ns, stat.st_size, expected_sha, "official")
        cached = source_cache.get(cache_key)
        if cached is not None:
            return cached
        try:
    ...
    def model_publish_parent_contract(selected: Path) -> dict[str, Any]:
        ...
        if is_grpo3_user_formal_manifest(manifest_data):
            try:
                declared_base = Path(str(manifest_data["base_model"])).expanduser().resolve()
            except (KeyError, OSError, TypeError, ValueError):
                declared_base = None
            registered_sha = next(
                (
                    digest for digest, path in model_publish_bases.items()
                    if declared_base is not None and path == declared_base
                ),
                None,
            )
            return {
                ...
                "source_checkpoint": manifest_data.get("parent_checkpoint_step"),
                "manifest": manifest_data,
            }
    ...
    def validate_publish_checkpoint(selected: Path, checkpoint: str) -> tuple[Path, dict[str, Any]]:
        ...
        if lineage.get("schema") == "grpo_adapter_lineage_v1":
        ...
        elif lineage.get("schema") == GRPO3_USER_LINEAGE_SCHEMA:
            checkpoint_match = PROMPT_CHECKPOINT_NAME_RE.fullmatch(checkpoint)
            checkpoint_step_value = int(checkpoint_match.group(1)) if checkpoint_match else None
            expected_source_checkpoint = contract.get("source_checkpoint")
            expected_source_sha = contract.get("source_adapter_sha256")
            ...
            continued_contract = (
                ...
                and lineage.get("config_sha256") == manifest_data.get("config_sha256")
                and lineage.get("code_commit") == manifest_data.get("git_commit")
                and lineage_base == contract.get("base_path")
            )
            if not continued_contract:
                raise HTTPException(status_code=409, detail="continued adapter lineage does not match this GRPO3 run")
        else:
    ...
    def download_checkpoint_file(checkpoint: str, file: str, run_id: str | None = None):
        ...
        for run_output in checkpoint_run_dirs(selected):
            ...
            if checkpoint_dir.parent == run_output and candidate.parent == checkpoint_dir and candidate.is_file():
                media_type = "application/json" if file == "adapter_config.json" else "application/octet-stream"
                return FileResponse(
                    candidate,
                    filename=f"{selected.name}-{checkpoint}-{file}",
    ...
    def capabilities(run_id: str | None = None):
        ...
        official_finegrained_v6 = (
            is_official_finegrained_v6_manifest(manifest_data)
            or (selected / "official_finegrained_v6.jsonl").is_file()
        )
        official_sample8_v3 = (
            is_official_sample8_v3_manifest(manifest_data)
            or (selected / "official_sample8.jsonl").is_file()
        )
        ...
        return {
            ...
            "official_finegrained_v6": official_finegrained_v6,
            "official_sample8_v3": official_sample8_v3,
            "advantage_formula": advantage_formula,
            "advantage_source": (
                "captured" if composite_formula or dual_beam8 or exact_sharpen_v4 or video_official_anticopy_v5 or official_anticopy_mixed_v5 or official_finegrained_v6 or official_sample8_v3
    ...
    def advantages(
        ...
        selected = selected_run(run_id)
        manifest_data = read_json(selected / "manifest.json", {})
        if is_official_sample8_v3_manifest(manifest_data):
            groups, optimization = adapt_official_sample8_v3_events(
                read_jsonl(selected / "official_sample8.jsonl"),
                from_step=from_step, to_step=to_step, rollout_id=rollout_id,
                group_id=group_id, limit=limit,
                source_index=official_source_rows(selected),
            )
            return official_sample8_v3_payload(groups, optimization)
        if is_official_finegrained_v6_manifest(manifest_data):
            groups, optimization = adapt_official_finegrained_v6_events(
                read_jsonl(selected / "official_finegrained_v6.jsonl"),
                from_step=from_step, to_step=to_step, rollout_id=rollout_id,
                group_id=group_id, limit=limit,
                source_index=official_source_rows(selected),
            )
            return official_finegrained_v6_payload(groups, optimization)
