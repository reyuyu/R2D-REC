(() => {
  ...
  function compactProbeCandidate(mode,candidate,index,golds){
    if(mode==='free')return `<tr><td>#${index+1}</td><td><b>${escapeHtml(sid(candidate.parsed_sid))}</b><details><summary>完整 Free 采样</summary>${probeCompletion(candidate)}<div class="v4-contract-note">parser ${escapeHtml(candidate.parser_status||'-')} · fixed domain prefix ${candidate.fixed_domain_prefix?'YES':'NO'}</div></details></td><td>${fmt(candidate.reward)}</td><td>${escapeHtml(candidate.parser_status||'-')}</td><td>${candidate.closed?'是':'否'}</td></tr>`;
    const beams=(candidate.beam_sids||[]).map((value,beamIndex)=>{const kind=sidKind(value,golds);return `<div class="v4-beam ${kind}"><b>#${beamIndex+1}</b> ${escapeHtml(sid(value))} · ${kind==='exact'?'Exact':kind==='ab'?'AB':kind==='a'?'A':kind==='invalid'?'Invalid':'Domain'}</div>`}).join('');
  ...
  function probeBlock(mode,data,golds){
    const free=mode==='free',rows=(data?.candidates||[]).map((candidate,index)=>compactProbeCandidate(mode,candidate,index,golds)).join('');
    return `<details class="probe-route" open><summary><h3 style="display:inline">${free?'Free Probe · 自由 continuation / 首个完整 SID':'Official Probe · fixed domain begin / production Beam32 ABC3'} · reward ${fmt(data?.reward_mean)} ± ${fmt(data?.reward_std)}</h3></summary><div class="v4-contract-note"><b>评分语义：</b>${escapeHtml(data?.reward_semantics||'-')}<br><b>分母：</b>${escapeHtml(data?.reward_denominator||'-')} · candidates ${data?.candidate_count??0} · closure ${pct(data?.closure_rate)}</div><div class="table-scroll"><table class="v4-table v4-probe-table"><thead><tr>${free?'<th>#</th><th>Parsed SID / 完整采样</th><th>Reward</th><th>Parser</th><th>Closed</th>':'<th>CoT / 完整文本</th><th>Reward</th><th>Exact / AB / A</th><th>Invalid</th><th>Beam32 明细</th>'}</tr></thead><tbody>${rows}</tbody></table></div></details>`
  }
  function renderV4Probes(){
    ...
    $('probeDetail').innerHTML=`<div class="trace-head">Step ${selected.step} · ${escapeHtml(selected.group_id)}</div><details class="v4-source" open><summary>Probe 模型输入原文 / Gold SID</summary><pre class="v4-cot-text">${escapeHtml(selected.think_prompt||'Probe 输入原文未记录')}</pre><div class="gold-strip"><span class="label">Gold SID</span>${(selected.gold_sids||[]).map(value=>`<span class="gold-chip">${escapeHtml(typeof value==='string'?value:sid(value))}</span>`).join('')}</div></details><div class="v4-consistency"><div class="v4-stat"><b>同一批 4 CoT</b><span>两路输入对齐</span></div><div class="v4-stat"><b>${pct(freePositive)}</b><span>Free A+ candidate rate</span></div><div class="v4-stat"><b>${pct(officialPositive)}</b><span>Official A+ Beam rate</span></div><div class="v4-stat"><b>${pct(Math.abs(freePositive-officialPositive))}</b><span>A+ rate 绝对差</span></div><div class="v4-stat"><b>${aliasOk?'PASS':'FAIL'}</b><span>think → Official alias</span></div></div><div class="v4-contract-note">两路共用 sampled CoT，但 continuation、候选分母与 reward 量纲不同；一致性用于比较趋势和覆盖，不能逐候选对齐。</div>${probeBlock('free',free,selected.gold_sids||[])}${probeBlock('official',official,selected.gold_sids||[])}`
  }
