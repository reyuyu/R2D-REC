...
RUN_ID="${GRPO_RUN_ID:-GR-USER-PILOT150-$(date +%Y%m%d-%H%M%S)}"
GIT_COMMIT="${PILOT_GIT_COMMIT:?PILOT_GIT_COMMIT is required}"
CONFIG="/data/GRPO_USER/config/pilot150_v1.json"
RUNNER="/data/GRPO_USER/scripts/run_user_pilot150.py"
MONITOR_ROOT="/data/GRPO/runs"

if [[ ! -f "$CONFIG" || ! -f "$RUNNER" ]]; then
  echo "Pilot150 config or runner is missing" >&2
  exit 1
fi
if [[ "$(nvidia-smi --query-gpu=index --format=csv,noheader | wc -l)" -ne 4 ]]; then
  echo "Pilot150 requires exactly four GPUs" >&2
  exit 1
fi
...
if [[ -e "/data/GRPO_USER/runs/$RUN_ID" || -e "$MONITOR_ROOT/$RUN_ID" ]]; then
