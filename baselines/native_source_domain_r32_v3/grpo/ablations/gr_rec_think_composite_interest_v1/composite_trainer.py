...
def score_candidate(completion, gold_cot, prompt, beam_raw):
    ...
    record = monitor_record(float(beam_raw), score, raw_n, grounded_n)
    record.update({
...
def build_global_runtime(records, beam_rewards, group_size=GROUP_SIZE):
    ...
    for start in range(0, len(records), group_size):
        ...
        scored = [score_candidate(row["completion"], row["gold_cot"], row["prompt"], reward)
    ...
    return {"candidates": candidates, "groups": groups, "advantages": advantages}
...
class ThinkCompositeInterestRecGRPOTrainer(RecGRPOTrainer):
    ...
    def _generate_and_score_completions(self, inputs):
        ...
        self._smoke_log[-1].update({
            "reward_definition": "0.60*beam_utility+0.40*cot_utility",
