"""Four-GPU zero-update preflight for Think Sample8 FullSID v3."""
...
from .run_sample8_fullsid_train import (
    FIXED_PROBE4_IDS, PARENT_ADAPTER, PARENT_ADAPTER_SHA256, Sample8ManifestWriter,
    assert_runtime_import_provenance, baseline_runner, make_sample8_grpo_config,
    prepare_sample8_run_plan, validate_parent_adapter,
)
...
def main(argv=None):
    ...
    parser.add_argument("--run-id", default="GR-REC-THINK-SAMPLE8-FULLSID-V3-PREFLIGHT-20260828")
    ...
    if world != 4:
        raise RuntimeError(f"SAMPLE8_FULLSID_PREFLIGHT_REQUIRES_4_GPU: {world}")
    ...
    plan = prepare_sample8_run_plan(plan_args)
    group_ids = list(plan["dataset"]["recommendation_group_id"])
    if PREFLIGHT_GROUP_ID not in group_ids:
        raise RuntimeError("SAMPLE8_FULLSID_PREFLIGHT_GROUP_MISSING")
    ...
    monitor = Sample8ManifestWriter(MonitorWriter(False, run_id=args.run_id, rank=rank))
    runtime = Sample8FullSIDRuntime(model, tokenizer)
    cfg = make_sample8_grpo_config("/tmp/grpo-sample8-fullsid-preflight", 2, 1e-6, SEED,
                                save_strategy="no", save_total_limit=None)
    trainer = ThinkSample8FullSIDTrainer(
    ...
    fingerprint_first = trainer._sample_rollout_fingerprint
    ...
    iteration2_reuse = (runtime.sample_calls == calls_after_first and
                        trainer._sample_rollout_fingerprint == fingerprint_first and cache_parity)
    ...
    production_cot_local = prepared["advantages"]
    production_sid = trainer._sid_rollout["advantages"]
    cot_grad = branch_backward(trainer, model, prepared, "cot", production_cot_local)
    sid_grad = branch_backward(trainer, model, prepared, "sid", production_sid)
    model.zero_grad(set_to_none=True)
    ids = torch.cat([prepared["prompt_ids"], prepared["completion_ids"]], 1)
    mask = torch.cat([prepared["prompt_mask"], prepared["completion_mask"]], 1)
    ...
    probe_monitor = Sample8ManifestWriter(MonitorWriter(True, probe_root, args.run_id, rank))
    ...
    if rank == 0:
        checks = {
            ...
            "no_natural_language_bridge": all(
                not row["natural_language_bridge"] for row in rank_rows),
            "four_independent_g8_not_g32": no_g32,
            "cot_reward_equals_sid_reward_sum": all(
                math.isclose(row["cot_reward"], sum(row["sid_rewards"]), abs_tol=1e-9)
                for row in rank_rows),
        ...
        payload = {
            "experiment": "GR_REC_ThinkSample8_FullSID_v3", "preflight_pass": not failures,
            ...
            "real_signal_observed": real_signal_observed,
            "gradient_boundary_mode": "immutable real sampled actions with production advantages",
