...
- Each CoT independently samples eight complete four-token SIDs with
  `do_sample=true, temperature=1, top_p=1, top_k=0,
  repetition_penalty=1, max_new_tokens=4`.
- Strict raw-ID parsing requires `domain_begin + A + B + C`; invalid output
  receives `q_reward=-1`.
...
- CoT loss updates sampled CoT action tokens only. SID loss updates all four
  sampled SID action tokens only.
