...
def parse_full_sid_ids(tokenizer, generated_ids):
    """Strictly parse raw domain+A+B+C token IDs, with no text fallback."""
    ids = list(generated_ids or [])
    if len(ids) != FULL_SID_TOKENS:
        return None
    tokens = tokenizer.convert_ids_to_tokens(ids, skip_special_tokens=False)
    if isinstance(tokens, str):
        tokens = [tokens]
    if len(tokens) != FULL_SID_TOKENS:
        return None
    domain = _DOMAIN_PATTERN(tokens[0])
    abc = [pattern(token) for pattern, token in zip(_ABC_PATTERNS, tokens[1:])]
    if domain is None or any(match is None for match in abc):
        return None
    return (domain.group(1), *(int(match.group(1)) for match in abc))
...
class Sample8FullSIDRuntime:
    ...
    def _sample_full_sids(self, context_ids):
        ...
        outputs = self.model.generate(
            ...
            max_new_tokens=FULL_SID_TOKENS,
            pad_token_id=pad_id,
        )
        generated = outputs[:, input_ids.size(1):]
        return [[int(token) for token in row.tolist()] for row in generated]
    ...
    def score_local_cot(self, prompt, cot_ids, gold_sids, target_domain, group_id=None):
        ...
        self.sample_calls += 1
        if len(generated_ids) != SID_G or any(
            len(ids) != FULL_SID_TOKENS for ids in generated_ids
        ):
            raise RuntimeError("SAMPLE8_FULLSID_NOT_EXACT_8XFULLSID4")
        sample_sids = [parse_full_sid_ids(self.tokenizer, ids) for ids in generated_ids]
        ...
        record = {
            ...
            "sampling_contract": {
                ...
                "top_k": 0, "repetition_penalty": 1.0, "max_new_tokens": 4,
            },
        }
...
class ThinkSample8FullSIDTrainer(RecGRPOTrainer):
    ...
    def _pad_sid_sequences(self, record):
        ...
        if len(candidates) != SID_G or any(
            len(candidate) != FULL_SID_TOKENS for candidate in candidates
        ):
            raise RuntimeError("Sample8 SID objective requires exactly 8xFullSID4")
        sequences = [
    ...
    def _build_sid_rollout(self, cot_output):
        ...
        with torch.no_grad():
            old_logps, _ = self._get_per_token_logps_and_entropies(
                self.model, input_ids, attention_mask, FULL_SID_TOKENS,
                compute_entropy=False)
        if old_logps.shape != (SID_G, FULL_SID_TOKENS) or old_logps.requires_grad:
    ...
    def _compute_loss(self, model, inputs, return_outputs=False, num_items_in_batch=None):
        ...
        sid_action_mask = torch.ones(
            (SID_G, FULL_SID_TOKENS), dtype=torch.long,
            device=sid["input_ids"].device)
