...
RUN_ID="${RUN_ID:-GR-REC-VIDEO-OFFICIAL-ANTICOPY-V5-WARMUP-FORMAL-E1-20260830}"
...
exec torchrun --nproc_per_node=4 --master_addr=127.0.0.1 --master_port="${MASTER_PORT}" \
  -m ablations.gr_rec_video_official_anticopy_v5_warmup.run_video_official_anticopy_train \
  --run-id "${RUN_ID}" \
  --n-groups all \
  --max-steps "${MAX_STEPS}" \
  --lr 1e-6 \
