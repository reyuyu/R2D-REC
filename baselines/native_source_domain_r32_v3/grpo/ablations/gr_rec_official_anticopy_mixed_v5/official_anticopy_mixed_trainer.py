"""V5-mixed: all-domain Think G4 plus independent Official Sample8 objectives."""
...
COPY_A_WARMUP_ROLLOUTS = 50


def fresh_rollout_index_from_step(global_step):
    step = int(global_step)
    if step < 0 or step % 2:
        raise RuntimeError(f"V5-mixed fresh rollout requires an even global step: {step}")
    return step // 2
...
def reward_stage(target_domain, fresh_rollout_index):
    if target_domain == "video":
        return "video_anticopy"
    if target_domain not in TARGET_DOMAINS:
        raise ValueError(f"unsupported target domain: {target_domain}")
    return (
        "nonvideo_copy_a_warmup"
        if int(fresh_rollout_index) < COPY_A_WARMUP_ROLLOUTS
        else "nonvideo_copy_a_off"
    )


def shape_sid_reward(sid, gold_sids, history_sids, target_domain,
                     fresh_rollout_index):
    """Apply the frozen domain-aware anti-copy reward for one candidate."""
    raw = float(q_reward(sid, gold_sids))
    copied = sid is not None and tuple(sid) in set(history_sids)
    if raw == -1.0:
        final = -1.0
    elif target_domain == "video":
        if raw == 8.0:
            final = 8.0
        elif copied:
            final = 0.0
        ...
        copy_rewards = {
            0.0: 0.0,
            0.5: 0.25 if int(fresh_rollout_index) < COPY_A_WARMUP_ROLLOUTS else 0.0,
            2.0: 1.0,
            8.0: 6.0,
        }
        final = copy_rewards.get(raw, raw)
    return raw, copied, final


def cot_candidate_contribution(target_domain, raw_q_reward, is_history_copy,
                               shaped_sid_reward):
    if target_domain == "video":
        return 0.0 if is_history_copy else float(raw_q_reward)
    return float(shaped_sid_reward)
...
def finalize_global_records(records, fresh_rollout_index):
    ...
    if len(domains) != 1 or not domains.issubset(TARGET_DOMAINS):
        raise RuntimeError(f"V5_MIXED_G4_DOMAIN_MIX: {sorted(domains)}")
    target_domain = next(iter(domains))
    stage = reward_stage(target_domain, fresh_rollout_index)
    for row in records:
        ...
        for sid in row["candidate_sids"]:
            raw, copied, final = shape_sid_reward(
                sid, gold, history, target_domain, fresh_rollout_index
            )
            raw_rewards.append(raw)
            copies.append(copied)
            sid_rewards.append(final)
            cot_contributions.append(cot_candidate_contribution(
                target_domain, raw, copied, final
            ))
...
def domain_rollout_summary(records):
    ...
    copy_advantages = [item["sid_advantage"] for item in copies]
    summary = {
        ...
        "shaped_reward_mean": (
            sum(item["final_sid_reward"] for item in details) / len(details)
            if details else 0.0
        ),
        "copy_positive_advantage_count": sum(
            item["is_history_copy"] and item["sid_advantage"] > 0
            for item in details
        ),
        "copy_mean_advantage": (
            sum(copy_advantages) / len(copy_advantages) if copy_advantages else 0.0
...
class OfficialAntiCopyMixedRuntime:
    ...
    def score_local_cot(self, prompt, cot_ids, gold_sids, target_domain, group_id):
        if target_domain not in TARGET_DOMAINS:
            raise RuntimeError(f"V5-mixed unsupported domain: {target_domain}")
...
def make_official_anticopy_mixed_reward_func(runtime):
    def reward_func(prompts, completions, completion_ids, **kwargs):
        ...
        if any(domain not in TARGET_DOMAINS for domain in kwargs.get("target_domain", [])):
            raise RuntimeError("V5-mixed requires a supported target domain")
    ...
    reward_func.__name__ = "official_anticopy_mixed_v5_cot_reward"
    reward_func.official_anticopy_mixed_runtime = runtime
    return reward_func


class ThinkOfficialAntiCopyMixedTrainer(RecGRPOTrainer):
    def __init__(self, *args, lambda_cot=LAMBDA_COT, lambda_sid=LAMBDA_SID, **kwargs):
        ...
        runtime = next((
            getattr(fn, "official_anticopy_mixed_runtime", None)
            for fn in funcs
            if getattr(fn, "official_anticopy_mixed_runtime", None) is not None
        ), None)
        if runtime is None:
            raise RuntimeError("V5 trainer requires OfficialAntiCopyMixedRuntime")
    ...
    def _prepare_inputs(self, batch):
        ...
        if rows and any(
            ...
            raise RuntimeError("V5-mixed dataset must be Think-only and all-domain")
    ...
    def _build_v5_rollout(self, cot_output):
        ...
        if self.accelerator.is_main_process and self._monitor.enabled:
            ...
            self._monitor.write_official_anticopy_mixed_v5({
    ...
    def log(self, logs, start_time=None):
        ...
        if (
            ...
            record = self._v5_rollout["record"] if self._v5_rollout else {}
