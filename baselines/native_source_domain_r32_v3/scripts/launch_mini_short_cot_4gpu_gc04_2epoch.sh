...
RUN_ID="${RUN_ID:-MINI-SHORT-COT-R32-2E-GC04-4GPU-$(date +%Y%m%d-%H%M%S)}"
SOURCE_CONFIG="${BASELINE_ROOT}/config/train_mini_short_cot_4gpu_gc04_2epoch.yaml"
DATASET_ROOT="/data/lf_data_versions/alltrain/mini_short_cot"
...
if [[ -e "${OUTPUT_DIR}" || -e "${LOG_DIR}" ]]; then
  echo "Refusing to reuse run path: ${RUN_ID}" >&2
  exit 1
fi
mkdir -p "${OUTPUT_DIR}/metadata" "${LOG_DIR}"
"${VENV_ROOT}/bin/python" "${BASELINE_ROOT}/scripts/preflight_mini_short_cot.py" >"${LOG_DIR}/preflight.log" 2>&1
...
cp "${DATASET_ROOT}/cache_audit.json" "${OUTPUT_DIR}/metadata/cache_audit.json"
cp "${SOURCE_CONFIG}" "${OUTPUT_DIR}/metadata/source_config.yaml"
