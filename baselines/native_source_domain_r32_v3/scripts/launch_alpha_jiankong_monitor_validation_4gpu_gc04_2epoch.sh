#!/usr/bin/env bash
# Formal entrypoint.  Do not invoke it until the user explicitly authorises training.
set -euo pipefail

BASELINE_ROOT="${BASELINE_ROOT:-/data/baselines/native_source_domain_r32_v3}"
RUN_ID="${RUN_ID:-ALPHA-JIANKONG-MONITOR-R32-2E-GC04-4GPU-$(date +%Y%m%d-%H%M%S)}"
SOURCE_CONFIG="${BASELINE_ROOT}/config/train_alpha_jiankong_monitor_validation_4gpu_gc04_2epoch.yaml"
...
# Fail closed before allocating GPUs or loading the model.
...
# Static launcher audit: validates the exact copied config and then exits before
# CUDA/NCCL/model initialization.  It is deliberately opt-in and never used by
# a normal formal launch.
if [[ "${ALPHA_FORMAL_PREFLIGHT_ONLY:-0}" == "1" ]]; then
  echo "ALPHA_FORMAL_PREFLIGHT_ONLY_PASS run_id=${RUN_ID}"
  exit 0
fi
