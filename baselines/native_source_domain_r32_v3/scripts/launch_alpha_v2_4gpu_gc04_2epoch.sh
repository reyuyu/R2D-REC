#!/usr/bin/env bash
# Alpha V2 formal entrypoint. Do not invoke without the user's explicit approval.
set -euo pipefail

BASELINE_ROOT="${BASELINE_ROOT:-/data/baselines/native_source_domain_r32_v3}"
RUN_ID="${RUN_ID:-ALPHA-V2-R32-2E-GC04-4GPU-$(date +%Y%m%d-%H%M%S)}"
SOURCE_CONFIG="${BASELINE_ROOT}/config/train_alpha_v2_4gpu_gc04_2epoch.yaml"
DATASET_ROOT="/data/lf_data_versions/alltrain/alpha_v2"
...
cp "${DATASET_ROOT}/dataset_info.json" "${OUTPUT_DIR}/metadata/dataset_info.json"
