#!/usr/bin/env bash
# Four-GPU smoke only; callback stops after 10 optimizer steps.
set -euo pipefail

BASELINE_ROOT="${BASELINE_ROOT:-/data/baselines/native_source_domain_r32_v3}"
RUN_ID="${RUN_ID:-MINI-TOPK-V1-SMOKE10-4GPU-$(date +%Y%m%d-%H%M%S)}"
SOURCE_CONFIG="${BASELINE_ROOT}/config/smoke_mini_topK_v1_4gpu_gc04_10step.yaml"
DATASET_ROOT="/data/lf_data_versions/alltrain/alpha_mini_v1"
...
cp "${DATASET_ROOT}/alpha_mini_v1_native_packing_audit.json" "${OUTPUT_DIR}/metadata/native_packing_audit.json"
cp "${DATASET_ROOT}/alpha_mini_v1_vs_alpha_validation_leakage.json" "${OUTPUT_DIR}/metadata/validation_leakage_audit.json"

exec env \
  CUDA_VISIBLE_DEVICES="${CUDA_VISIBLE_DEVICES:-0,1,2,3}" \
  NPROC_PER_NODE=4 \
  ALLOW_EXTRA_ARGS=1 \
  PACK_RATIO_STOP_AFTER_STEPS=10 \
  SOURCE_WEIGHT_SKIP_FINAL_SAVE=1 \
  MINI_TOPK_STEP0_DIAGNOSTICS=1 \
