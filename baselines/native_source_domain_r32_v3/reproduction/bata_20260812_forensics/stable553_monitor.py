...
<section class="band seeds-only"><h2>Seed 实验汇总</h2><div class="hint section-note">每行对应一个随机种子；Epoch 1 为共同父 checkpoint，Epoch 2 为该 seed 的独立续训。</div><div class="tablewrap"><table class="run-table"><thead><tr><th>Seed</th><th>Epoch 1</th><th>Epoch 2</th><th>Epoch 1 成绩</th><th>Epoch 2 成绩</th><th>备注</th><th>操作</th></tr></thead><tbody id="epoch2Rows"><tr><td colspan="7" class="hint">尚无续训记录</td></tr></tbody></table></div></section>
...
function renderSeedStats(){const box=document.getElementById('seedStats'),run=selectedSeedRun();if(!run){box.innerHTML='<div class="waiting">尚无随机种子实验</div>';return}const s=run.statistics||{},latest=run.latest_scalar||{},progress=Math.max(0,(run.step-553)/553),e1=run.manual_score?.epoch1_score,e2=run.manual_score?.epoch2_score??run.manual_score?.score;box.innerHTML=`<div class="grid">${metric('Seed',run.seed)}${metric('续训完成度',(progress*100).toFixed(1)+'%')}${metric('记录点数',s.logged_point_count??0)}${metric('Evidence',run.evidence_count+'/4')}${metric('Epoch 1 成绩',e1??'—')}${metric('Epoch 2 成绩',e2??'—')}${metric('Loss mean',f(s.loss_mean,5))}${metric('Loss min',f(s.loss_min,5))}${metric('Loss max',f(s.loss_max,5))}${metric('Grad mean',f(s.grad_norm_mean,5))}${metric('Grad max',f(s.grad_norm_max,5))}${metric('最新 epoch',f(latest.epoch,4))}</div><div class="hint section-note"><code>${esc(run.run_id)}</code> · Parent ${esc(run.source_label||'STABLE553-A')} · Epoch 2 Adapter ${run.download_ready?'可下载':'训练完成后可下载'}</div>`}
function renderEpoch2(){if(!state)return;const rows=state.epoch2_runs||[],body=document.getElementById('epoch2Rows'),active=rows.some(x=>x.status==='RUNNING'),button=document.getElementById('launchEpoch2');syncSeedSelection(rows);button.disabled=active||!state.epoch2_launch_ready;document.getElementById('launchMessage').textContent=launchMessage||(active?'训练进行中；完成或停止后才能启动下一组 seed。':state.epoch2_launch_ready?'四张 GPU 空闲，可以启动。':'GPU 当前不可用于新训练。');if(!rows.length){body.innerHTML='<tr><td colspan="7" class="hint">尚无续训记录</td></tr>';renderSeedStats();return}rows.forEach(x=>{if(!scoreDrafts[x.run_id])scoreDrafts[x.run_id]={epoch1_score:x.manual_score?.epoch1_score??'',epoch2_score:x.manual_score?.epoch2_score??x.manual_score?.score??'',notes:x.manual_score?.notes??''}});body.innerHTML=rows.map(x=>{const d=downloadStates[x.run_id]||{},parentDownload=downloadStates['STABLE553-A']||{},draft=scoreDrafts[x.run_id],latest=x.latest_scalar||{},busy=!!d.busy;return `<tr class="${x.run_id===selectedRunId?'selected':''}"><td><b>${x.seed}</b></td><td><b>STABLE553-A</b><div class="hint">checkpoint-553 · PASS</div></td><td><code>${esc(x.run_id)}</code><div><span class="run-status ${x.status.toLowerCase()}">${x.status}</span> · ${x.step}/1106 · Loss ${f(latest.loss,4)} · Grad ${f(latest.grad_norm,4)}</div></td><td><input class="score-input" aria-label="Seed ${x.seed} Epoch 1 成绩" type="number" step="0.0001" value="${esc(draft.epoch1_score)}" oninput="scoreDrafts['${x.run_id}'].epoch1_score=this.value"></td><td><input class="score-input" aria-label="Seed ${x.seed} Epoch 2 成绩" type="number" step="0.0001" value="${esc(draft.epoch2_score)}" oninput="scoreDrafts['${x.run_id}'].epoch2_score=this.value"></td><td><input class="notes-input" maxlength="500" value="${esc(draft.notes)}" oninput="scoreDrafts['${x.run_id}'].notes=this.value"></td><td><button type="button" onclick="selectSeedRun('${x.run_id}')">查看</button> <button type="button" onclick="saveScore('${x.run_id}')">保存成绩</button> <button type="button" ${state.runs[0]?.download_ready&&!parentDownload.busy?'':'disabled'} onclick="downloadAdapter('STABLE553-A')">下载 E1</button> <button type="button" ${x.download_ready&&!busy?'':'disabled'} onclick="downloadEpoch2Adapter('${x.run_id}')">${busy?'下载中…':'下载 E2'}</button><div class="download-state">${esc(d.text||parentDownload.text||'')}</div></td></tr>`}).join('');renderSeedStats()}
