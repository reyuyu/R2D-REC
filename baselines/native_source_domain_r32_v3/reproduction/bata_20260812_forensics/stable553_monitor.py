...
SEED_RUN_RE = re.compile(r"^SEED-S([0-9]{1,10})-([0-9]{8}-[0-9]{6})$")
MAX_SEED = 2**31 - 1
MIN_SEED_FREE_BYTES = 20 * 1024**3
...
def seed_checkpoint_download_path(root: Path, run_id: str, epoch: int, filename: str) -> Path | None:
    if SEED_RUN_RE.fullmatch(run_id) is None or epoch not in (1, 2) or filename not in DOWNLOAD_FILES:
        return None
    step = 553 if epoch == 1 else 1106
    path = root / "seed_runs" / run_id / "output" / f"checkpoint-{step}" / filename
    return path if path.is_file() else None
...
def save_manual_score(
    ...
    notes: str,
    *,
    epoch1_score: float | None = None,
    epoch2_score: float | None = None,
) -> dict[str, Any]:
    legacy = isinstance(run_id, str) and EPOCH2_RUN_RE.fullmatch(run_id) is not None
    fresh = isinstance(run_id, str) and SEED_RUN_RE.fullmatch(run_id) is not None
    run_dir = root / ("epoch2_runs" if legacy else "seed_runs") / str(run_id)
    if not (legacy or fresh) or not run_dir.is_dir():
        raise ValueError("unknown seed run")
    if epoch2_score is None:
        epoch2_score = score
    for name, value in (("epoch1_score", epoch1_score), ("epoch2_score", epoch2_score)):
    ...
    record = {
        "epoch1_score": float(epoch1_score) if epoch1_score is not None else None,
        "epoch2_score": float(epoch2_score) if epoch2_score is not None else None,
        "score": float(epoch2_score) if epoch2_score is not None else None,
        "notes": notes.strip(),
    }
...
def epoch2_run_status(root: Path, run_dir: Path, scores: dict[str, dict[str, Any]]) -> dict[str, Any]:
    ...
    if complete and evidence_pass:
        ...
        status = "STOPPED"
    return {
        "experiment_kind": "legacy_epoch2_continuation",
        "run_id": run_id,
...
def _evidence_rows(path: Path) -> list[dict[str, Any]]:
    rows = []
    for item in sorted(path.glob("runtime_rank*.json")):
        try:
...
def seed_run_status(root: Path, run_dir: Path, scores: dict[str, dict[str, Any]]) -> dict[str, Any]:
    run_id = run_dir.name
    try:
        manifest = json.loads((run_dir / "manifest.json").read_text(encoding="utf-8"))
    except (OSError, json.JSONDecodeError, TypeError):
        return {"run_id": run_id, "experiment_kind": "fresh_seed", "status": "INVALID", "step": 0}
    initial_log = run_dir / "train.log"
    resume_log = run_dir / "resume.log"
    initial_text = initial_log.read_text(encoding="utf-8", errors="replace") if initial_log.is_file() else ""
    resume_text = resume_log.read_text(encoding="utf-8", errors="replace") if resume_log.is_file() else ""
    initial_series = scalar_rows(initial_text)
    resume_series = scalar_rows(resume_text, start_step=553)
    merged = {int(row["step"]): row for row in (*initial_series, *resume_series)}
    scalar_series = [merged[step] for step in sorted(merged)]
    initial_pid = _read_pid(run_dir / "launcher.pid")
    resume_pid = _read_pid(run_dir / "resume_launcher.pid")
    initial_evidence = _evidence_rows(run_dir / "evidence_initial")
    resume_evidence = _evidence_rows(run_dir / "evidence_resume")
    initial_pass = len(initial_evidence) == 4 and all(row.get("status") == "PASS" for row in initial_evidence)
    resume_pass = len(resume_evidence) == 4 and all(row.get("status") == "PASS" for row in resume_evidence)
    checkpoint1 = run_dir / "output/checkpoint-553"
    checkpoint2 = run_dir / "output/checkpoint-1106"
    epoch1_complete = _checkpoint_complete(checkpoint1)
    epoch2_complete = _checkpoint_complete(checkpoint2)
    early_stop = bool(manifest.get("early_stop_epoch1"))
    initial_alive = process_alive(initial_pid)
    resume_alive = process_alive(resume_pid)
    explicit_steps = [int(value) for value in STEP_RE.findall(initial_text + "\n" + resume_text)]
    step = max([0, *explicit_steps, *(int(row["step"]) for row in scalar_series)], default=0)
    step = min(step, 1106)
    if epoch2_complete and ((early_stop and resume_pass) or (not early_stop and initial_pass)):
        status, step = "PASS", 1106
    elif initial_alive or resume_alive:
        status = "RUNNING"
    elif early_stop and epoch1_complete and initial_pass and resume_pid is None:
        status, step = "EPOCH1_PASS", 553
    elif initial_pid is None:
        status = "READY"
    else:
        status = "STOPPED"
    errors = [
        line.strip() for line in (initial_text + "\n" + resume_text).replace("\r", "\n").splitlines()
    ...
    return {
        ...
        "step": step,
        "target_step": 1106,
        "requested_epochs": int(manifest.get("requested_epochs", 2)),
        "early_stop_epoch1": early_stop,
        "initial_launcher_pid": initial_pid,
        "resume_launcher_pid": resume_pid,
        "epoch1_checkpoint_complete": epoch1_complete,
        "epoch2_checkpoint_complete": epoch2_complete,
        "checkpoint_complete": epoch2_complete,
        "download_ready": epoch2_complete,
        "evidence_count": len(initial_evidence) + len(resume_evidence),
        "initial_evidence_count": len(initial_evidence),
        "resume_evidence_count": len(resume_evidence),
        "can_resume": status == "EPOCH1_PASS",
        "latest_scalar": scalar_series[-1] if scalar_series else None,
        "series": scalar_series,
        "statistics": series_statistics(scalar_series),
        "errors": errors,
...
def seed_runs(root: Path) -> list[dict[str, Any]]:
    runs_root = root / "seed_runs"
    scores = manual_scores(root)
    fresh = [] if not runs_root.is_dir() else [
        seed_run_status(root, path, scores)
        for path in sorted(runs_root.iterdir(), reverse=True)
        if path.is_dir() and SEED_RUN_RE.fullmatch(path.name)
    ]
    return fresh + epoch2_runs(root)


def storage_status(root: Path) -> dict[str, Any]:
    usage = shutil.disk_usage(root)
    return {
        "path": str(root / "seed_runs"),
        "total_bytes": usage.total,
        "used_bytes": usage.used,
        "free_bytes": usage.free,
        "minimum_free_bytes": MIN_SEED_FREE_BYTES,
        "ready": usage.free >= MIN_SEED_FREE_BYTES,
    }
...
def _active_training(root: Path) -> bool:
    return any(row["status"] == "RUNNING" for row in seed_runs(root))


def _launch_seed_phase(root: Path, run_id: str, phase: str) -> dict[str, Any]:
    support = Path(__file__).resolve().parent
    launcher = support / "launch_stable_seed.sh"
    run_dir = root / "seed_runs" / run_id
    log_name = "train.log" if phase == "initial" else "resume.log"
    pid_name = "launcher.pid" if phase == "initial" else "resume_launcher.pid"
    log_handle = (run_dir / log_name).open("ab", buffering=0)
    try:
        process = subprocess.Popen(
            ["bash", str(launcher), str(root), run_id, phase],
            cwd=support,
            stdin=subprocess.DEVNULL,
            stdout=log_handle,
            stderr=subprocess.STDOUT,
        ...
        log_handle.close()
    (run_dir / pid_name).write_text(f"{process.pid}\n", encoding="utf-8")
    return {"status": "STARTED", "run_id": run_id, "phase": phase, "pid": process.pid}


def start_seed_run(root: Path, seed: int, early_stop_epoch1: bool = False) -> dict[str, Any]:
    if isinstance(seed, bool) or not isinstance(seed, int) or not 0 <= seed <= MAX_SEED:
        raise ValueError(f"seed must be an integer in [0, {MAX_SEED}]")
    if not isinstance(early_stop_epoch1, bool):
        raise ValueError("early_stop_epoch1 must be boolean")
    if _active_training(root):
    ...
    if not storage["ready"]:
        raise RuntimeError("/root seed_runs 可用空间不足 20 GiB")
    run_id = f"SEED-S{seed}-{datetime.now().strftime('%Y%m%d-%H%M%S')}"
    support = Path(__file__).resolve().parent
    command = [
        "/data/venvs/llamafactory-01398eb-liger081/bin/python",
        str(support / "prepare_stable_seed.py"),
        "prepare",
        "--root", str(root),
        "--run-id", run_id,
        "--seed", str(seed),
    ]
    if early_stop_epoch1:
        command.append("--early-stop")
    subprocess.run(command, check=True, stdout=subprocess.PIPE, stderr=subprocess.PIPE, text=True)
    result = _launch_seed_phase(root, run_id, "initial")
    return {**result, "seed": seed, "early_stop_epoch1": early_stop_epoch1}


def resume_seed_run(root: Path, run_id: str) -> dict[str, Any]:
    if not isinstance(run_id, str) or SEED_RUN_RE.fullmatch(run_id) is None:
        raise ValueError("invalid seed run id")
    if _active_training(root):
        raise RuntimeError("已有训练正在运行")
    rows = {row["run_id"]: row for row in seed_runs(root)}
    row = rows.get(run_id)
    if row is None or row.get("experiment_kind") != "fresh_seed":
        raise ValueError("unknown fresh seed run")
    if not row.get("can_resume"):
        raise RuntimeError("该实验不处于可续训的 Epoch 1 完成状态")
    ready, reason = gpu_launch_ready()
    if not ready:
        raise RuntimeError(reason)
    if not storage_status(root)["ready"]:
        raise RuntimeError("/root seed_runs 可用空间不足 20 GiB")
    return _launch_seed_phase(root, run_id, "resume")
...
def snapshot(root: Path) -> dict[str, Any]:
    ...
    return {
        ...
        "scalars": scalars,
        "series": all_series,
        "seed_experiments": continuations,
        "epoch2_runs": continuations,
        "epoch2_active_run": next((row["run_id"] for row in continuations if row["status"] == "RUNNING"), None),
        "epoch2_launch_ready": gpu_launch_ready()[0] and not _active_training(root),
        "seed_launch_ready": gpu_launch_ready()[0] and not _active_training(root) and storage_status(root)["ready"],
        "storage": storage_status(root),
...
.launchbar{display:flex;align-items:flex-end;gap:10px;flex-wrap:wrap}.field{display:flex;flex-direction:column;gap:4px}.field input{font:inherit;border:1px solid #aebcc0;border-radius:4px;padding:8px 10px;width:180px}.checkfield{display:flex;align-items:center;gap:7px;min-height:38px;padding:0 5px}.checkfield input{width:16px;height:16px;margin:0}.primary{background:var(--green);border-color:var(--green);color:#fff}.primary:hover{background:#067653;color:#fff}.launch-message{min-height:20px;margin-top:8px;font-size:12px}.tablewrap{overflow-x:auto;margin-top:14px}.run-table{width:100%;border-collapse:collapse;min-width:1080px}.run-table th,.run-table td{text-align:left;padding:9px 8px;border-bottom:1px solid var(--line);vertical-align:middle}.run-table th{font-size:12px;color:var(--muted)}.run-table input{font:inherit;border:1px solid #bdc8cb;border-radius:3px;padding:6px 7px}.score-input{width:90px}.notes-input{width:210px}.run-status{font-weight:700}.run-status.running{color:var(--blue)}.run-status.pass,.run-status.epoch1_pass{color:var(--green)}.run-status.stopped,.run-status.invalid{color:var(--red)}
...
<section class="band seeds-only"><div class="charthead"><div><h2>启动全新 Seed 训练</h2><div class="hint">每次从 BASE 独立初始化；默认训练 2 Epoch。早停仍保留 1106-step 调度，并保存可完整恢复的 checkpoint-553。</div></div><div class="launchbar"><label class="field"><span class="label">随机种子</span><input id="seedInput" type="number" min="0" max="2147483647" step="1" value="20260806"></label><label class="checkfield"><input id="earlyStopEpoch1" type="checkbox"><span>Epoch 1 早停</span></label><button id="launchSeed" class="primary" type="button">从 BASE 训练 2 Epoch</button></div></div><div id="launchMessage" class="launch-message hint"></div><div id="storageMessage" class="hint"></div></section>
...
<section class="band seeds-only"><h2>Seed 实验汇总</h2><div class="hint section-note">新实验从 BASE 开始；早停实验可从同一行续训 1 Epoch。旧记录明确标为“历史续训”，不会混作 fresh run。</div><div class="tablewrap"><table class="run-table"><thead><tr><th>Seed</th><th>Epoch 1</th><th>Epoch 2</th><th>Epoch 1 成绩</th><th>Epoch 2 成绩</th><th>备注</th><th>操作</th></tr></thead><tbody id="epoch2Rows"><tr><td colspan="7" class="hint">尚无 Seed 记录</td></tr></tbody></table></div></section>
...
const f=(v,n=6)=>v==null?'—':Number(v).toFixed(n);let state=null,smoothN=1,view={min:0,max:PAGE==='seeds'?15:25,auto:true},drag=null,launchMessage='',selectedRunId=null;const downloadStates={},scoreDrafts={};
...
function selectedSeedRun(){return state?.seed_experiments?.find(x=>x.run_id===selectedRunId)||null}
function seriesLabels(){return PAGE==='seeds'?(selectedRunId?[selectedRunId]:[]):BASE_LABELS}
function colorFor(label){return COLORS[label]||(label.startsWith('EPOCH2-')?'#8a6b55':'#aa6710')}
function allSeries(key){const src=state?.series||{};return seriesLabels().map(label=>({label,rows:smooth(src[label]||[],key,smoothN)}))}
function pageBounds(){const run=selectedSeedRun();return PAGE==='seeds'?{min:run?.experiment_kind==='legacy_epoch2_continuation'?553:0,max:1106,minSpan:5}:{min:0,max:553,minSpan:15}}
...
function selectSeedRun(runId){selectedRunId=runId;syncSeedSelection(state.seed_experiments||[]);renderTop();renderEpoch2();renderSeedStats();renderCurveHeader();resetView()}
function renderSeedStats(){const box=document.getElementById('seedStats'),run=selectedSeedRun();if(!run){box.innerHTML='<div class="waiting">尚无随机种子实验</div>';return}const s=run.statistics||{},latest=run.latest_scalar||{},e1=run.manual_score?.epoch1_score,e2=run.manual_score?.epoch2_score??run.manual_score?.score,kind=run.experiment_kind==='fresh_seed'?'BASE 独立训练':'历史续训（非从头）';box.innerHTML=`<div class="grid">${metric('Seed',run.seed)}${metric('训练来源',kind)}${metric('当前 step',run.step+'/1106')}${metric('记录点数',s.logged_point_count??0)}${metric('Epoch 1 成绩',e1??'—')}${metric('Epoch 2 成绩',e2??'—')}${metric('Loss mean',f(s.loss_mean,5))}${metric('Loss min',f(s.loss_min,5))}${metric('Loss max',f(s.loss_max,5))}${metric('Grad mean',f(s.grad_norm_mean,5))}${metric('Grad max',f(s.grad_norm_max,5))}${metric('最新 epoch',f(latest.epoch,4))}</div><div class="hint section-note"><code>${esc(run.run_id)}</code> · ${kind} · ${run.early_stop_epoch1?'Epoch 1 早停合同':'默认 2 Epoch'} · checkpoint-553 ${run.epoch1_checkpoint_complete||run.experiment_kind!=='fresh_seed'?'可用':'未完成'} · checkpoint-1106 ${run.download_ready?'可用':'未完成'}</div>`}
function checkpointButton(run,epoch){const fresh=run.experiment_kind==='fresh_seed',ready=fresh?(epoch===1?run.epoch1_checkpoint_complete:run.epoch2_checkpoint_complete):(epoch===1?state.runs[0]?.download_ready:run.download_ready),key=run.run_id+'-e'+epoch,d=downloadStates[key]||{},disabled=!ready||d.busy;return `<button type="button" ${disabled?'disabled':''} onclick="downloadSeedAdapter('${run.run_id}',${epoch})">${d.busy?'下载中…':'下载 E'+epoch}</button>`}
function renderEpoch2(){if(!state)return;const rows=state.seed_experiments||[],body=document.getElementById('epoch2Rows'),active=rows.some(x=>x.status==='RUNNING'),button=document.getElementById('launchSeed'),early=document.getElementById('earlyStopEpoch1');syncSeedSelection(rows);button.disabled=active||!state.seed_launch_ready;early.disabled=button.disabled;document.getElementById('storageMessage').textContent=state.storage?`保存目录：${state.storage.path} · 可用 ${(state.storage.free_bytes/1024/1024/1024).toFixed(1)} GiB · 最低要求 20 GiB`:'';document.getElementById('launchMessage').textContent=launchMessage||(active?'已有训练正在运行；不会中断它。':state.seed_launch_ready?'四张 GPU 空闲，可以从 BASE 启动新实验。':'GPU 或存储当前不满足新训练条件。');if(!rows.length){body.innerHTML='<tr><td colspan="7" class="hint">尚无 Seed 记录</td></tr>';renderSeedStats();return}rows.forEach(x=>{if(!scoreDrafts[x.run_id])scoreDrafts[x.run_id]={epoch1_score:x.manual_score?.epoch1_score??'',epoch2_score:x.manual_score?.epoch2_score??x.manual_score?.score??'',notes:x.manual_score?.notes??''}});body.innerHTML=rows.map(x=>{const draft=scoreDrafts[x.run_id],latest=x.latest_scalar||{},fresh=x.experiment_kind==='fresh_seed',kind=fresh?'从 BASE':'历史续训';return `<tr class="${x.run_id===selectedRunId?'selected':''}"><td><b>${x.seed}</b><div class="hint">${kind}</div></td><td><b>${fresh?(x.epoch1_checkpoint_complete?'checkpoint-553':'训练中/待生成'):'共同父 checkpoint-553'}</b><div class="hint">${fresh?'该 seed 独立 Epoch 1':'非该 seed 从头训练'}</div></td><td><code>${esc(x.run_id)}</code><div><span class="run-status ${x.status.toLowerCase()}">${x.status}</span> · ${x.step}/1106 · Loss ${f(latest.loss,4)} · Grad ${f(latest.grad_norm,4)}</div></td><td><input class="score-input" aria-label="Seed ${x.seed} Epoch 1 成绩" type="number" step="0.0001" value="${esc(draft.epoch1_score)}" oninput="scoreDrafts['${x.run_id}'].epoch1_score=this.value"></td><td><input class="score-input" aria-label="Seed ${x.seed} Epoch 2 成绩" type="number" step="0.0001" value="${esc(draft.epoch2_score)}" oninput="scoreDrafts['${x.run_id}'].epoch2_score=this.value"></td><td><input class="notes-input" maxlength="500" value="${esc(draft.notes)}" oninput="scoreDrafts['${x.run_id}'].notes=this.value"></td><td><button type="button" onclick="selectSeedRun('${x.run_id}')">查看</button> <button type="button" onclick="saveScore('${x.run_id}')">保存成绩</button> ${checkpointButton(x,1)} ${checkpointButton(x,2)} ${x.can_resume?`<button class="primary" type="button" onclick="resumeSeed('${x.run_id}')">续训 1 Epoch</button>`:''}</td></tr>`}).join('');renderSeedStats()}
...
async function launchSeed(){const seed=Number(document.getElementById('seedInput').value),early=document.getElementById('earlyStopEpoch1').checked;if(!Number.isInteger(seed)||seed<0||seed>2147483647){launchMessage='Seed 必须是 0 到 2147483647 的整数。';renderEpoch2();return}const scope=early?'Epoch 1 早停（之后可续训）':'完整 2 Epoch';if(!confirm(`从 BASE 启动全新四卡训练？\nSeed: ${seed}\n范围: ${scope}`))return;launchMessage='正在验证 BASE、配置、存储与 GPU 合同…';renderEpoch2();try{const result=await postJson('/api/seed/start',{seed,early_stop_epoch1:early});launchMessage='已启动 fresh run：'+result.run_id;selectedRunId=result.run_id;await refresh()}catch(e){launchMessage='启动失败：'+e.message;renderEpoch2()}}
async function resumeSeed(runId){if(!confirm(`从 ${runId} 的完整 checkpoint-553 续训 1 Epoch？`))return;launchMessage='正在校验 optimizer、scheduler、trainer 与四份 RNG 状态…';renderEpoch2();try{await postJson('/api/seed/resume',{run_id:runId});launchMessage='已续训：'+runId;await refresh()}catch(e){launchMessage='续训失败：'+e.message;renderEpoch2()}}
async function saveScore(runId){const draft=scoreDrafts[runId],epoch1Score=draft.epoch1_score===''?null:Number(draft.epoch1_score),epoch2Score=draft.epoch2_score===''?null:Number(draft.epoch2_score);if((epoch1Score!==null&&!Number.isFinite(epoch1Score))||(epoch2Score!==null&&!Number.isFinite(epoch2Score))){launchMessage='两轮成绩必须是数字或留空。';renderEpoch2();return}try{await postJson('/api/epoch2/score',{run_id:runId,epoch1_score:epoch1Score,epoch2_score:epoch2Score,notes:draft.notes});launchMessage='两轮成绩已保存：'+runId;await refresh()}catch(e){launchMessage='成绩保存失败：'+e.message;renderEpoch2()}}
...
function downloadSeedAdapter(runId,epoch){const run=state.seed_experiments.find(x=>x.run_id===runId),key=runId+'-e'+epoch;if(run.experiment_kind==='fresh_seed')return downloadBundle(key,name=>`/api/seed/checkpoint/${encodeURIComponent(runId)}/${epoch}/${name}`);if(epoch===1)return downloadBundle(key,name=>`/api/checkpoint/STABLE553-A/${name}`);return downloadBundle(key,name=>`/api/epoch2/checkpoint/${encodeURIComponent(runId)}/${name}`)}
function renderTop(){const status=document.getElementById('status'),metrics=document.getElementById('metrics');if(PAGE==='seeds'){const run=selectedSeedRun(),s=run?.latest_scalar||{},ok=run&&!['STOPPED','INVALID'].includes(run.status);status.textContent=run?`${run.status==='RUNNING'?'训练中':run.status} · Seed ${run.seed}`:'尚无随机种子实验';status.className='status '+(ok?'healthy':run?'bad':'');document.getElementById('overallText').textContent=run?`${run.step} / 1106`:'—';document.getElementById('overallBar').style.width=run?(100*run.step/1106)+'%':'0';document.getElementById('progressHint').textContent=run?.experiment_kind==='fresh_seed'?'该曲线从 BASE 的 step 0 开始；Epoch 1 早停可原状态续训。':'该行是历史 checkpoint-553→1106 续训，不是 fresh seed。';metrics.innerHTML=metric('当前 optimizer step',run?.step??'—')+metric('最新 logging step',s.step??'—')+metric('Loss',f(s.loss))+metric('Grad norm',f(s.grad_norm))+metric('Learning rate',s.learning_rate==null?'—':Number(s.learning_rate).toExponential(4))+metric('Epoch',f(s.epoch,4))}else{const complete=state.runs.every(x=>x.status==='PASS'),last=(state.series?.['STABLE553-B']||[]).at(-1)||{};status.textContent=complete?'A/B 已完成 · 权重比较已生成':state.active_run?`${state.active_run} 训练中`:'等待启动';status.className='status '+(complete||state.active_run?'healthy':'');document.getElementById('overallText').textContent=state.overall_step+' / '+state.overall_target;document.getElementById('overallBar').style.width=(100*state.overall_step/state.overall_target)+'%';document.getElementById('progressHint').textContent='总体进度按两个独立 run 各 553 个 optimizer updates 计算。';metrics.innerHTML=metric('总 optimizer updates',state.overall_step)+metric('A 状态',state.runs[0]?.status||'—')+metric('B 状态',state.runs[1]?.status||'—')+metric('B 最终 Loss',f(last.loss))+metric('B 最终 Grad',f(last.grad_norm))+metric('比较结果',state.comparison?.status||'—')}}
function renderCurveHeader(){const run=selectedSeedRun(),range=run?.experiment_kind==='legacy_epoch2_continuation'?'553–1106':'0–1106';document.getElementById('curveTitle').textContent=PAGE==='seeds'?'独立 Seed 训练曲线':'A/B 首轮训练曲线';document.getElementById('curveLegend').innerHTML=PAGE==='seeds'?(run?`<span><i style="background:${colorFor(run.run_id)}"></i>Seed ${run.seed} · ${esc(run.run_id)}</span>`:''):'<span><i style="background:#07865f"></i>STABLE553-A</span><span><i style="background:#2369a9"></i>STABLE553-B</span>';document.getElementById('curveHint').textContent=PAGE==='seeds'?`仅展示当前实验。滚轮缩放 step ${range}，按住拖动平移，双击重置。`:'滚轮缩放 step 0–553，按住拖动平移，双击重置。数据来自 logging_steps=5。'}
async function refresh(){try{const r=await fetch('/api/status',{cache:'no-store'}),d=await r.json(),previousMax=view.max;state=d;syncSeedSelection(d.seed_experiments||[]);document.getElementById('clock').textContent='实时 · '+new Date().toLocaleTimeString();renderTop();renderRuns();renderEpoch2();renderComparison();renderCurveHeader();document.getElementById('gpus').innerHTML=d.gpus.map(g=>`<div class="gpurow"><b>GPU ${g.index}</b><div class="bar"><i style="width:${100*g.memory_used/g.memory_total}%"></i></div><span>${g.memory_used} MiB</span><span>${g.utilization}%</span></div>`).join('');if(view.auto){const bounds=pageBounds(),steps=seriesLabels().flatMap(label=>(d.series?.[label]||[]).map(x=>x.step)),mx=Math.max(bounds.min+bounds.minSpan,...steps);view.min=bounds.min;view.max=Math.min(bounds.max,Math.max(previousMax,mx))}drawAll()}catch(e){document.getElementById('status').textContent='监控连接失败';document.getElementById('status').className='status bad'}}
bindChart('loss');bindChart('grad');document.getElementById('smooth').addEventListener('change',e=>{smoothN=Number(e.target.value);drawAll()});document.getElementById('resetZoom').addEventListener('click',resetView);document.getElementById('launchSeed').addEventListener('click',launchSeed);document.getElementById('earlyStopEpoch1').addEventListener('change',e=>{document.getElementById('launchSeed').textContent=e.target.checked?'从 BASE 训练 1 Epoch':'从 BASE 训练 2 Epoch'});document.getElementById('seedRunSelect').addEventListener('change',e=>selectSeedRun(e.target.value));refresh();setInterval(refresh,5000);addEventListener('resize',drawAll);
</script></body></html>"""
    ...
    class Handler(BaseHTTPRequestHandler):
        ...
        def do_GET(self):
            ...
            if len(parts) == 5 and parts[:3] == ["api", "epoch2", "checkpoint"]:
                ...
                self.send_file(path)
                return
            if len(parts) == 6 and parts[:3] == ["api", "seed", "checkpoint"]:
                try:
                    epoch = int(parts[4])
                except ValueError:
                    self.send_error(404)
                    return
                path = seed_checkpoint_download_path(root, parts[3], epoch, parts[5])
                if path is None:
                    self.send_error(404)
                    return
                self.send_file(path)
        ...
        def do_POST(self):
            ...
            try:
                ...
                with TRAIN_ACTION_LOCK:
                    if request_path == "/api/epoch2/start":
                    ...
                    elif request_path == "/api/seed/start":
                        result = start_seed_run(
                            root,
                            body.get("seed"),
                            body.get("early_stop_epoch1", False),
                        )
                        self.send_json(202, result)
                    elif request_path == "/api/seed/resume":
                        result = resume_seed_run(root, body.get("run_id"))
                        self.send_json(202, result)
                        ...
                        record = save_manual_score(
                            ...
                            body.get("score"),
                            body.get("notes", ""),
                            epoch1_score=body.get("epoch1_score"),
                            epoch2_score=body.get("epoch2_score", body.get("score")),
                        )
