...
def build(source: Path, historical_manifest: Path, canonical_rows: Path, output: Path) -> dict[str, Any]:
    ...
    split = {
        ...
        "primary_prompt_family": dict(Counter(f"{row['target_domain']}|{row['prompt_family']}" for row in primary)),
        "primary_gold_count_bucket": dict(Counter(f"{row['target_domain']}|{'K1' if row['gold_count'] == 1 else 'K2PLUS'}" for row in primary)),
