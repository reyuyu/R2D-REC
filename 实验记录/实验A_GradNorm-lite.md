# 实验 A：GradNorm-lite 多任务梯度平衡
## 竞赛评分更新（物料评分已可信）

本节自 2026-08-04 起采用完整十一项分项与平台总分进行比较。早期“懂物料不具备参考性”的备注只反映当时的评测状态。

| 实验 | checkpoint | 平台总分 | 懂物料（4 项） | 懂用户（2 项） | 懂推荐（4 项） | 懂世界 |
| --- | ---: | ---: | --- | --- | --- | ---: |
| 实验 A | 4500 | 1.2061 | 0.0450, 0.0380, 0.0455, 0.0411 | 0.1517, 0.0982 | 0.0709, 0.1734, 0.1708, 0.1368 | 0.2346 |
| 实验 A | 5200（最终） | 1.2054 | 0.0459, 0.0375, 0.0453, 0.0423 | 0.1558, 0.0977 | 0.0756, 0.1666, 0.1680, 0.1377 | 0.2331 |
| rank16 基础多任务分组（无 GradNorm、无冲突监控） | 5200（最终，重评） | 1.2151 | 0.0461, 0.0369, 0.0416, 0.0415 | 0.1483, 0.1003 | 0.0803, 0.1734, 0.1694, 0.1431 | 0.2342 |

最终 checkpoint 的当前评分口径下，实验 A 比 rank16 基线低 0.0097。此前表内较早的基线物料分数属于旧评分口径，不能与这条重评记录混用。
...
| **实验 A** | **3500** | **1.0217** | 0.0006, 0.0069, 0.0030, 0.0000 | 0.1363, 0.0930 | 0.0700, 0.1734, 0.1680, 0.1368 | 0.2338 | **1.0113** | - |
...
当前结果表明，实验 A 在 checkpoint-1000 的可比较分数高于基础实验，checkpoint-2500 时两者基本持平；
实验 A checkpoint-3500 的可比较分数为 1.0113，但缺少同 checkpoint 的基础实验结果，因此不作同阶段增益判断。
