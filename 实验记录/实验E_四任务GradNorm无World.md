...
实验 E 训练中。以下为 1000 checkpoint 的评测结果；评分仍按物料四域、用户两项、推荐四域、world 的固定顺序输出。world 已从训练任务移除，但仍保留在统一外部评测中。
...
单个 checkpoint 不能判断四任务拆分与无 world 训练是否优于基线；需待后续 checkpoint 在相同口径下比较。
