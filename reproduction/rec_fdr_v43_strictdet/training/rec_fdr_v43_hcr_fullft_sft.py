#!/usr/bin/env python3
"""V4.3-HCR continuation trainer derived from the frozen V4.2-A trainer.

V4.2-A's CoT, multi-positive objective, FDR, curriculum, packing, and task
normalization are preserved. HCR is a feature-gated additive recommendation
auxiliary and never launches a candidate-specific Transformer forward.
"""
...
CORE_JOINT_METRICS = (
    "rec", "rec_base_v42", "sid_set", "a_set", "ab_set", "fd_total", "fd_loss_a", "fd_loss_b", "fd_loss_c",
...
JOINT_METRIC_NAMES = CORE_JOINT_METRICS + MODE_METRICS + DOMAIN_MODE_METRICS + HCR_METRIC_NAMES
...
V43_CONFIG: dict[str, Any] = {}
HCR_CONFIG: dict[str, Any] = {"enabled": False}
V44_CONFIG: dict[str, Any] = {"enabled": False}
V21_LABELS = (
    "EXACT_RESCUE", "AB_RESCUE", "A_RESCUE", "FRONTIER_BOOST",
    "BOTH_HIT", "NEUTRAL", "HARMFUL", "MIXED_RESCUE_REGRESSION",
    "EXCLUDED_FROM_BEAM", "UNMAPPED",
)
BASE_TRAIN = [
...
def metric_reduction_keys() -> list[str]:
    ...
    return sorted({
        ...
        *(f"v21_label/{label}" for label in V21_LABELS),
        *(f"v21_nt_strength/{strength}" for strength in ("strong", "weak")),
        "effective_r2_weight/MIXED",
        "effective_r2_weight/MIXED_den",
    })


def v44_r2_weighted_mean(
    losses: torch.Tensor, base_weights: torch.Tensor, policy_weights: torch.Tensor
) -> torch.Tensor:
    """Preserve S1's 1/K denominator while scaling MIXED contribution by 0.5."""
    if losses.numel() != base_weights.numel() or losses.numel() != policy_weights.numel():
        raise ValueError("V4.4 R2 loss and weight vectors must have identical lengths")
    return (losses * base_weights * policy_weights).sum() / base_weights.sum().clamp_min(1e-8)


def v44_policy_factor(policy: dict[str, Any], *, is_full_batch: bool) -> float:
    """Validate one reviewed V21 policy entry at the runtime loss boundary."""
    label = str(policy["v21_primary_label"])
    weight = float(policy["v21_policy_weight"])
    if label == "HARMFUL":
        raise RuntimeError("V4.4 HARMFUL training exposure detected")
    if is_full_batch and label == "MIXED_RESCUE_REGRESSION":
        raise RuntimeError("V4.4 MIXED Full exposure detected")
    expected = 0.5 if label == "MIXED_RESCUE_REGRESSION" else 1.0
    if weight != expected:
        raise RuntimeError(f"unexpected V4.4 policy weight for {label}: {weight}")
    return weight


def validate_v44_step_override(v44: dict[str, Any], custom_max: int, *, smoke: bool) -> None:
    """Keep the reviewed 475-step formal budget while allowing exactly two smoke steps."""
    if not v44.get("enabled"):
        return
    if smoke:
        if custom_max != 2:
            raise ValueError("V4.4 smoke must use exactly 2 steps")
        return
    if custom_max not in {0, int(v44["frozen_s1_optimizer_steps"])}:
        raise ValueError("V4.4 formal max-steps override must be absent or exactly 475")
...
def load_v43_config(path: Path) -> dict[str, Any]:
    """Load a V4.3 YAML, resolving the review configs' local `_base_` chain."""
    loaded = OmegaConf.load(path)
    base_name = loaded.get("_base_")
    if base_name:
        del loaded["_base_"]
        merged = OmegaConf.merge(OmegaConf.create(load_v43_config(path.parent / str(base_name))), loaded)
    else:
        merged = loaded
...
def _patch_collator_trainer_and_model(
    ...
    hcr_metadata: HCRMetadata | None = None
    v21_policy_by_group: dict[int, dict[str, Any]] = {}
    if joint_cfg.get("enabled"):
        ...
        if HCR_CONFIG.get("enabled"):
            metadata_path = Path(str(HCR_CONFIG.get("metadata_path", "")))
            if not metadata_path.is_file():
                raise FileNotFoundError(f"V4.3 HCR metadata sidecar is missing: {metadata_path}")
            hcr_metadata = HCRMetadata.load(metadata_path)
            missing = set(group_catalog) - set(hcr_metadata.groups)
            if missing:
                raise ValueError(f"HCR metadata is missing {len(missing)} recommendation groups")
        if V44_CONFIG.get("enabled"):
            policy_path = Path(str(V44_CONFIG.get("policy_path", "")))
            if not policy_path.is_file():
                raise FileNotFoundError(f"V4.4 V21 policy sidecar is missing: {policy_path}")
            policy_raw = json.loads(policy_path.read_text(encoding="utf-8"))
            if int(policy_raw.get("schema_version", 0)) != 1:
                raise ValueError("V4.4 V21 policy sidecar must use schema_version=1")
            v21_policy_by_group = {
                int(group_index): dict(value)
                for group_index, value in policy_raw.get("groups", {}).items()
            }
            missing_policy = train_group_ids - set(v21_policy_by_group)
            if missing_policy:
                raise ValueError(f"V4.4 policy is missing {len(missing_policy)} S1 train groups")
            for group_index in train_group_ids:
                policy = v21_policy_by_group[group_index]
                label = str(policy.get("v21_primary_label", ""))
                weight = float(policy.get("v21_policy_weight", -1.0))
                expected = 0.5 if label == "MIXED_RESCUE_REGRESSION" else 1.0
                if label == "HARMFUL":
                    expected = 1.0  # Catalog provenance remains, but data exposure must be zero.
                if label not in V21_LABELS or weight != expected:
                    raise ValueError(f"invalid V4.4 policy for group {group_index}: {policy}")
    ...
    def get_curriculum_dataset(template: Any, model_args: Any, data_args: Any, training_args: Any, **kwargs: Any) -> dict[str, Any]:
        ...
        observed_baseline_budget = base_budget + full_budget
        baseline_budget = observed_baseline_budget
        base_fraction = base_budget / baseline_budget
        if V44_CONFIG.get("enabled"):
            baseline_budget = int(V44_CONFIG["frozen_s1_target_tokens"])
            base_fraction = float(V44_CONFIG["frozen_s1_base_fraction"])
        ...
        plan_report.update(
            {
                ...
                "observed_filtered_baseline_nonpad_tokens": observed_baseline_budget,
    ...
    def load_weighted_model(*args: Any, **kwargs: Any) -> Any:
        ...
        def forward(
            ...
            **model_kwargs: Any,
            ...
            v21_label_counts: Counter[str] = Counter()
            v21_nt_strength_counts: Counter[str] = Counter()
            mixed_policy_weights: list[float] = []
            ...
            if joint_cfg.get("enabled") and bool(rec_groups.any()):
                ...
                for instance in torch.unique(shifted_rec_instances[rec_groups]).tolist():
                    ...
                    policy_factor = 1.0
                    if V44_CONFIG.get("enabled") and group_index in train_group_ids:
                        policy = v21_policy_by_group[group_index]
                        label = str(policy["v21_primary_label"])
                        strength = str(policy["v21_nt_strength"])
                        try:
                            policy_factor = v44_policy_factor(policy, is_full_batch=is_full_batch)
                        except RuntimeError as exc:
                            raise RuntimeError(f"{exc}: group_index={group_index}") from exc
                        if not is_full_batch and label == "MIXED_RESCUE_REGRESSION":
                            mixed_policy_weights.append(policy_factor)
                        if label in V21_LABELS:
                            v21_label_counts[label] += 1
                        if strength in {"strong", "weak"}:
                            v21_nt_strength_counts[strength] += 1
                    ...
                    for name, value in set_losses.items():
                        cfg = joint_cfg[name]
                        if cfg["enabled"]:
                            parts.append(float(cfg["weight"]) * value)
                            active_sum += float(cfg["weight"])
                    for name, value in (("first_divergence_rank", fdr_loss),):
                        cfg = joint_cfg[name]
                        if cfg["enabled"]:
                            parts.append(float(cfg["weight"]) * value)
                            active_sum += float(cfg["weight"])
                    rec_loss_v42 = torch.stack(parts).sum() / active_sum
                    hcr_metrics: dict[str, torch.Tensor] = {}
                    if HCR_CONFIG.get("enabled"):
                        if hcr_metadata is None:
                            raise RuntimeError("HCR enabled without a loaded metadata sidecar")
                        inventory = (
                            domain_inventory if group_index in train_group_ids else validation_domain_inventory
                        )[group_domains[group_index]]
                        hcr_aux, hcr_metrics = compute_hcr_loss(
                            stage_logits=stage_logits,
                            anchor=(truth["a"], truth["b"], truth["c"]),
                            positives=positives,
                            train_inventory=inventory,
                            metadata=hcr_metadata.groups[group_index],
                            behavior_reliability=hcr_metadata.behavior_reliability,
                            config=HCR_CONFIG,
                            seed=seed + group_index,
                        )
                        rec_loss = rec_loss_v42 + hcr_aux
                        if float(hcr_metrics["video_multi_a_eligible"].detach()) > 0.0:
                            multi_a_eligible_group_ids.add(group_index)
                        if float(hcr_metrics["video_multi_a_active"].detach()) > 0.0:
                            multi_a_active_instances += 1
                    else:
                        rec_loss = rec_loss_v42
                        if not torch.equal(rec_loss, rec_loss_v42):
                            raise AssertionError("HCR-off recommendation loss diverged from V4.2")
                        hcr_metrics = {
                            "hcr_off_loss_delta": (rec_loss - rec_loss_v42).abs().detach()
                        }
                    instance_losses.append(rec_loss)
                    instance_modes.append(mode)
                    instance_domains.append(group_domains[group_index])
                    base_instance_weight = (
                        (0.5 if mode == "cot" else 1.0)
                        if is_full_batch else 1.0 / len(positives)
                    )
                    instance_base_weights.append(base_instance_weight)
                    instance_weights.append(base_instance_weight * policy_factor)
                    item_metrics = {
                        **set_losses,
                        **anchor_nll,
                        "fd_total": fdr_loss,
                        "rec": rec_loss,
                        "rec_base_v42": rec_loss_v42,
                        **fdr_metrics,
                        **hcr_metrics,
                    }
                    for name, value in item_metrics.items():
                        ...
                        if mode in {"cot", "uncot"}:
                            mode_key = f"{mode}_{name}"
                            if mode_key in MODE_METRICS:
                                metric_lists[mode_key].append(value.detach())
                ...
                base_weights = torch.tensor(instance_base_weights, device=shifted_hidden.device)
                if is_full_batch:
                    ...
                    # The denominator intentionally excludes the V21 policy factor.
                    # This preserves S1's 1/K group normalization while making a
                    # MIXED group contribute exactly half the baseline magnitude.
                    rec_batch_loss = v44_r2_weighted_mean(
                        torch.stack(instance_losses),
                        base_weights,
                        weights / base_weights.clamp_min(1e-8),
                    )
                    loss = rec_batch_loss
                for name, values in metric_lists.items():
                    stacked = torch.stack(values)
                    joint_metrics[name] = (
                        torch.median(stacked)
                        if name.startswith("catalog_rank_pack_median_")
                        else stacked.mean()
                    )
                metric_reference = instance_losses[0].detach()
                unique_eligible = len(multi_a_eligible_group_ids)
                joint_metrics["video_multi_a_active_instances_per_pack"] = metric_reference.new_tensor(
                    float(multi_a_active_instances)
                )
                joint_metrics["video_multi_a_unique_eligible_groups_per_pack"] = metric_reference.new_tensor(
                    float(unique_eligible)
                )
                joint_metrics["video_multi_a_repeat_factor"] = metric_reference.new_tensor(
                    multi_a_active_instances / unique_eligible if unique_eligible else 0.0
                )
            ...
            LAST_CURRICULUM_METRICS = {
                ...
                "v21_label_counts": dict(v21_label_counts),
                "v21_nt_strength_counts": dict(v21_nt_strength_counts),
                "mixed_policy_weight_sum": sum(mixed_policy_weights),
                "mixed_policy_weight_count": len(mixed_policy_weights),
            }
            return result
    ...
    def record(self: Any, metrics: dict[str, Any], evaluating: bool) -> None:
        ...
        for label, value in metrics.get("v21_label_counts", {}).items():
            accum[f"v21_label/{label}"] += float(value)
        for strength, value in metrics.get("v21_nt_strength_counts", {}).items():
            accum[f"v21_nt_strength/{strength}"] += float(value)
        accum["effective_r2_weight/MIXED"] += float(metrics.get("mixed_policy_weight_sum", 0.0))
        accum["effective_r2_weight/MIXED_den"] += float(metrics.get("mixed_policy_weight_count", 0.0))
        ...
        if not evaluating:
            ...
            for label, value in metrics.get("v21_label_counts", {}).items():
                self._task_stage_counts[f"v21_label/{label}"] += float(value)
            for strength, value in metrics.get("v21_nt_strength_counts", {}).items():
                self._task_stage_counts[f"v21_nt_strength/{strength}"] += float(value)
            self._task_stage_counts["effective_r2_weight/MIXED"] += float(metrics.get("mixed_policy_weight_sum", 0.0))
            self._task_stage_counts["effective_r2_weight/MIXED_den"] += float(metrics.get("mixed_policy_weight_count", 0.0))
    ...
    def log(self: Any, logs: dict[str, float], *args: Any, **kwargs: Any) -> None:
        if "loss" in logs and self._task_train_accum:
            ...
            if V44_CONFIG.get("enabled"):
                for label in V21_LABELS:
                    logs[f"train_v21_label/{label}"] = counts.get(f"v21_label/{label}", 0.0)
                for strength in ("strong", "weak"):
                    logs[f"train_v21_nt_strength/{strength}"] = counts.get(
                        f"v21_nt_strength/{strength}", 0.0
                    )
                mixed_den = counts.get("effective_r2_weight/MIXED_den", 0.0)
                logs["effective_r2_weight/MIXED"] = (
                    counts.get("effective_r2_weight/MIXED", 0.0) / mixed_den
                    if mixed_den else 0.0
                )
...
def _jsonl_callback(output_dir: Path, smoke: bool = False) -> Any:
    ...
    disable_intermediate_checkpoints = os.environ.get("REC_DISABLE_INTERMEDIATE_CHECKPOINTS") == "1"
    ...
    class JsonlCallback(TrainerCallback):
        ...
        def on_train_begin(self, args: Any, state: Any, control: Any, **kwargs: Any) -> None:
            ...
            if not smoke:
                intermediate_milestones = {max(1, round(state.max_steps * value)) for value in (.2, .4, .8)}
                self.eval_milestones = intermediate_milestones | {max(1, int(state.max_steps))}
                if not disable_intermediate_checkpoints:
                    self.save_milestones = intermediate_milestones
                ...
                if state.is_world_process_zero:
                    print(
                        json.dumps(
                            {
                                "event": "checkpoint_policy",
                                "intermediate_checkpoints_enabled": not disable_intermediate_checkpoints,
                                "save_milestones": sorted(self.save_milestones),
                                "eval_milestones": sorted(self.eval_milestones),
                            }
                        ),
                        flush=True,
                    )
...
def main() -> None:
    global REC_JOINT_CONFIG, LEGAL_SID_UNIVERSE, STOP_AT_PROGRESS, DOMAIN_CONDITIONED_FULL_C2
    global V42_CONFIG, V43_CONFIG, HCR_CONFIG, V44_CONFIG
    cli = parse_args()
    raw = load_v43_config(cli.config)
    ...
    V42_CONFIG = dict(v42)
    v43 = raw.pop("v43_experiment", None)
    valid_initializations = {"from_pretrain", "v42a_continuation_pilot"}
    if (
        not isinstance(v43, dict)
        or v43.get("family") != "hcr"
        or v43.get("initialization") not in valid_initializations
    ):
        raise ValueError(f"v43_experiment.initialization must be one of {sorted(valid_initializations)}")
    role = str(v43.get("role", ""))
    if role == "formal" and v43.get("initialization") != "from_pretrain":
        raise ValueError("formal V4.3 HCR experiments must initialize from pretrain")
    if role == "continuation_pilot" and v43.get("initialization") != "v42a_continuation_pilot":
        raise ValueError("continuation_pilot must use the V4.2-A continuation initialization")
    V43_CONFIG = dict(v43)
    hcr = raw.pop("hcr", None)
    if not isinstance(hcr, dict):
        raise ValueError("V4.3 requires an explicit hcr configuration block")
    HCR_CONFIG = dict(hcr)
    validate_hcr_config(HCR_CONFIG)
    validate_hcr_stage_contract(str(V43_CONFIG.get("stage", "")), HCR_CONFIG)
    v44 = raw.pop("v44_utility_filter", None)
    if v44 is not None:
        V44_CONFIG = dict(v44)
        required_v44 = {
            "enabled": True,
            "policy_version": "v1",
            "mixed_r2_policy_weight": 0.5,
            "harmful_r2_action": "drop",
            "domain_rebalance": False,
            "frozen_s1_target_tokens": 242367498,
            "frozen_s1_optimizer_steps": 475,
        }
        for key, expected in required_v44.items():
            if V44_CONFIG.get(key) != expected:
                raise ValueError(f"v44_utility_filter.{key} must equal {expected!r}")
        if V43_CONFIG.get("stage") != "s1" or V43_CONFIG.get("initialization") != "from_pretrain":
            raise ValueError("V4.4 utility filtering must use formal S1 from-pretrain")
        if HCR_CONFIG["history_fdr"]["enabled"] or HCR_CONFIG["multi_a"]["enabled"]:
            raise ValueError("V4.4 freezes S1: History-FDR and Multi-A must both be disabled")
        frozen_base_fraction = float(V44_CONFIG.get("frozen_s1_base_fraction", -1.0))
        if not math.isclose(frozen_base_fraction, 173862521 / 242367498, rel_tol=0.0, abs_tol=1e-15):
            raise ValueError("v44_utility_filter.frozen_s1_base_fraction must match the reviewed S1 plan")
        for key in ("reviewed_dataset_manifest_sha256", "reviewed_feasibility_sha256"):
            value = str(V44_CONFIG.get(key, ""))
            if re.fullmatch(r"[0-9a-f]{64}", value) is None:
                raise ValueError(f"v44_utility_filter.{key} must be a lowercase SHA256")
    ...
    validate_v44_step_override(V44_CONFIG, custom_max, smoke=cli.smoke)
    ...
    if cli.validate_only:
        ...
        # The review container may not expose CUDA, so only this non-training
        # parse uses CPU-safe dtype flags; the resolved formal config is not
        # mutated and still requires BF16 + FA2 on the four-GPU launcher.
        validation_raw = dict(parser_raw)
        validation_raw["bf16"] = False
        validation_raw["use_cpu"] = True
        model_args, data_args, training_args, finetuning_args, _ = _parse_train_args(validation_raw)
        print(
            json.dumps(
                {
                    ...
                    "v43_experiment": V43_CONFIG,
                    "hcr": HCR_CONFIG,
                    "v44_utility_filter": V44_CONFIG,
                },
                ensure_ascii=False,
            )
        )
